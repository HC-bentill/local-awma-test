...
class Business extends CI_Controller {
	...
	public function add_business_property_form(){
		...
		if(has_permission($this->session->userdata('user_info')['id'],'create buis prop')){
			$data = array(
				...
				'division' => $this->res->get_divisions(),
				'class' => $this->res->get_class(),
				'zone' => $this->res->get_zone(),
				'usecode' => $this->res->get_usecode(),
			);
	...
	public function add_business_occupant_form(){
		...
		if(has_permission($this->session->userdata('user_info')['id'],'create buis occ')){
			$data = array(
				...
				'display_category' => $this->res->get_display_category()
			);
	...
	public function add_business(){
		...
		$property_category = $this->input->post('category');
		
		
		$primary_contact = trim($this->input->post('primary_contact'));
		$gen_rescode = strtoupper(trim($this->input->post('property_code')));
		...
		$owner['houseno'] = trim($this->input->post('houseno'));
		...
		$data['block_no'] = trim($this->input->post('blockno'));
		$data['plot_no'] = trim($this->input->post('parcelno'));
		
		$data['category'] = 12;
		$data['property_type'] = trim($this->input->post('category'));
		$data['accessed'] = trim($this->input->post('accessed_status'));
		$data['zone_code'] = trim($this->input->post('zone_code'));
		$data['property_typee'] = trim($this->input->post('use_code'));
		$data['class_code'] = trim($this->input->post('class_code'));
		...
		if(!$bus_id && !$owner_id && !$bus_to_owner){
        	...
        	if($echannel != 0){
        		$sms_message = "Your Property has been registered successfully on the ". SYSTEM_ID ." Platform.\nYour Property Code is $gen_rescode\nThank You";
	...
	public function add_business_occupant(){
		$town = get_town_name($this->input->post('town'));
		...
		$gen_rescode = strtoupper(trim($this->input->post('business_code')));
		...
		$data['buis_property_code'] = strtoupper(trim($this->input->post('buis_property_code')));
		...
		$data['ghpost_gps'] = trim($this->input->post('ghpost_gps'));
		$data['division'] = trim($this->input->post('division'));
		$data['bus_category'] = trim($this->input->post('display_category'));
	...
	public function edit_business_info_data(){
		...
		//$data['old_bus_code'] = trim($this->input->post('old_bus_code'));
		$data['buis_property_code'] = strtoupper(trim($this->input->post('buis_property_code')));
		$data['buis_occ_code'] = strtoupper(trim($this->input->post('business_code')));
		$data['bus_category'] = trim($this->input->post('display_category'));
	...
	public function edit_business_location_data(){
		$rescode = $this->input->post('bus_code');
		$town = get_town_name($this->input->post('town'));
		...
		$res_id = $this->res->update_business_occ($data,$resid);
		...
		if(!$res_id){
			...
			$info = array(
				'user_id' => $this->session->userdata('user_info')['id'],
				'activity' => "Edited a business location data",
				'status' => true
			);
			$audit_tray = audit_tray($info);
			//end of insert
		...
		redirect(base_url().'Business/edit_business_occupant_form/'.$resid);
	...
	public function edit_personnal_data(){
		...
		if(!$owner_id){
			...
			$info = array(
				'user_id' => $this->session->userdata('user_info')['id'],
				'activity' => "Edited property owner data",
				'status' => true,
				'description' => "Edited property owner info with code: $rescode",
				'user_category' => "admin",
				'channel' => "Web"
			);
			$audit_tray = audit_tray($info);
	...
	public function edit_loc_data(){
		...
		if(!$res_id){
			...
			$info = array(
				'user_id' => $this->session->userdata('user_info')['id'],
				'activity' => "Edited a property location data",
				'status' => true
			);
			$audit_tray = audit_tray($info);
			//end of insert
	...
	public function edit_prop_data(){
		...
		$rescode = $this->input->post('busprop');
		$data['buis_prop_code'] = trim($this->input->post('property_code'));
		$data['category'] = 12;
		$data['property_type'] = trim($this->input->post('category'));
		$data['accessed'] = trim($this->input->post('accessed_status'));
		$data['zone_code'] = trim($this->input->post('zone_code'));
		...
		if(!$res_id){
			...
			$info = array(
				'user_id' => $this->session->userdata('user_info')['id'],
				'activity' => "Edited a property data",
				'status' => true,
				'description' => "Edited property data with code: $rescode",
				'user_category' => "admin",
				'channel' => "Web"
			);
			$audit_tray = audit_tray($info);
	...
	public function search_business_occ_code(){
		$search_value = strtoupper($this->uri->segment(3));
		$query = $this->res->get_business_occ_code($search_value);
		echo json_encode($query);
	}
