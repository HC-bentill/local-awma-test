...
class Invoice extends CI_Controller {
	...
	public function print_batch_invoice(){
		...
		$data = array(
			'result' => json_decode($this->TaxModel->get_batch_invoices($product,$category1,$year,$electoral_area,$town,$offset,$division)),
			'product' => $product,
			'category1' => $category1,
			'year' => $year,
			'electoral_area' => $electoral_area,
	...
	// special batch print invoice
	public function special_batch(){
		// $where_in = array("AYW04155003","AYW04155001","AYW04155002");
		$product = $_POST['product'];
		$property_code = $_POST['property_codes'];

		$where_in = explode(" ", $property_code);
		$year = $_POST['year'];
		$data = array(
			'result' => json_decode($this->TaxModel->get_special_batch_invoices($where_in,$year)),
			// 12 & 13 = property , BOP = 1
			'product' => $product
		);
		$this->load->view('invoices/special_print_batch',$data);
	}

	//api to fetch prop codes from vw_invoice based on billType ID
	public function get_all_prop_codes(){
        // POST data
        $postdata = $this->input->post();
        // get data
        $data = $this->TaxModel->get_all_prop_codes($postdata);
        echo json_encode($data);
    }

	public function import_special_batch_records()
	{
		$count = 0;
		$successful = 0;
		$failed = 0;
		...
		if (!$this->upload->do_upload()) {
				...
				redirect(base_url().'special_batch_print_invoice');
		} else {
			if(!empty($_FILES['userfile']['name']) && in_array($_FILES['userfile']['type'],$csvMimes)){
				...
				if(is_uploaded_file($_FILES['userfile']['tmp_name'])){
					...
					while(($line = fgetcsv($csvFile)) !== FALSE){
						$raw_data = "";
						$row = 0;
						$code = $this->TaxModel->get_special_batch_count();
						$final_code =  $code + 1;
						$number = str_pad($final_code, 4, '0', STR_PAD_LEFT);
						$batch_no = "BAT".$number;
						$data['batch_no'] = $batch_no;
						$data['product'] = trim($line[0]);
						$data['year'] = trim($line[1]);
						$data['property_code'] = $line[2];
						$data['user_id'] = $this->session->userdata('user_info')['id'];
						$save = $this->TaxModel->save_special_batch($data);
						$count++;

						if($save){
							$info = array(
								'user_id' => $this->session->userdata('user_info')['id'],
								'activity' => "Imported Special Batch Invoice Records",
								'status' => false,
								'user_category' => "admin",
							...
							$this->session->set_flashdata('message',"<div class='alert alert-success'>Your Import was Successful !</div>");
							redirect(base_url().'special_batch_print_invoice');
						}else{
	...
	public function batch_print_invoice(){
		...
		if(has_permission($this->session->userdata('user_info')['id'],'batch_print_invoice')){
			$data = array(
				...
				'area' => $this->TaxModel->get_area_councils(), 
				'products' => $this->TaxModel->get_all_products(),
				'division' => $this->TaxModel->get_divisions(),
			);
	...
	//laod special batch print invoice view
	public function special_batch_print_invoice_view(){
		$this->session->set_userdata('last_page', 'special_batch_print_invoice');
		buildBreadCrumb(array(
			"url" => "special_batch_print_invoice",
			"label" => "Special Batch Print Invoice"
		), TRUE);

		if(has_permission($this->session->userdata('user_info')['id'],'special_batch_print_invoice')){
			$data = array(
				'title' => 'Special Batch Print Invoice',
				'page' => 'invoices/special_batch_print_invoice',
				'result' => $this->TaxModel->get_special_batches(),
				'area' => $this->TaxModel->get_area_councils(), 
				'products' => $this->TaxModel->get_all_products(),
				'division' => $this->TaxModel->get_divisions(),
				'property_codes' => $this->TaxModel->get_property_codes()
			);
	...
	public function save_batch_print_invoice(){
		...
		$category1 = $_POST['category1'];
		$division = $_POST['division'];
		if($this->TaxModel->batch_exit($product,$category1,$year,$electoral_area,$town,$division)){
			$this->session->set_flashdata('message', "<div class='alert alert-danger'>
    ...
    public function save_special_batch_print_invoice(){
        $code = $this->TaxModel->get_special_batch_count();
        $final_code =  $code + 1;
        $number = str_pad($final_code, 4, '0', STR_PAD_LEFT);
        $batch_no = "BAT".$number;
        $year = $_POST['year'];
        $product = $_POST['product'];
        $property_codes = $_POST['prop_codes'];
		
        // if($property_codes == ""){
            
        // }else{
        //     $prop_code = implode(" ", $property_codes);
        // }
        // $division = $_POST['division'];
        // if($this->TaxModel->special_batch_exist($product,$year,$division)){
        //  $this->session->set_flashdata('message', "<div class='alert alert-danger'>
        //          <strong>Sorry! </strong>This batch already exist.
        //  </div>");
        //  redirect(base_url('special_batch_print_invoice_view'));
        ...
        if($save){
            ...
            $this->session->set_flashdata('message', "<div class='alert alert-danger'>
                    <strong>Sorry! </strong>An error occured whilst saving batch.
            </div>");
            redirect(base_url('special_batch_print_invoice'));
        }
