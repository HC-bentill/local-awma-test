...
// old send an sms api
function old_send_sms($phone, $message) {
...
// send an sms
function send_sms2($phone, $message) {
    $url = "http://api.nalosolutions.com/bulksms/?username=pmsult&password=pmconsult123&type=0&dlr=1&destination=";
    $url .= $phone . "&source=".SYSTEM_ID."&message=" . rawurlencode($message);
...
function send_sms($phone, $message) {
    $url = "https://sms.nalosolutions.com/smsbackend/clientapi/Resl_nalo/send-message/?username=deksolops&password=deks@l@ps&type=0&dlr=1&destination=";
    $url .= $phone . "&source=".SYSTEM_ID."&message=" . rawurlencode($message);
...
// get area council code
function get_last_year_arrears_payment($property_id,$product_id,$invoice_year)
{
    $CI = &get_instance();
    $query = $CI->db->query("SELECT amount_paid, invoice_amount from invoice WHERE property_id = $property_id AND product_id = $product_id
              AND invoice_year = '$invoice_year'")->row_array();
    return $query;
}
...
function get_batch_print_invoice($product,$category1, $year, $electoral_area, $town, $division)
{
    $CI = & get_instance();
    $CI->db->select('count(*) as total_count');
    $CI->db->from('vw_invoice as i');
    $CI->db->where('i.invoice_amount <> 0');
    ...
    ($division) ? $CI->db->where('i.division', $division) : null;

    $total = $CI->db->get()->row_array();
    return $total['total_count'];
}

//count of all records selected
function get_special_batch_count($year,$property_code){
    $CI = & get_instance();
    $CI->db->select('count(*) as total_count');
    $CI->db->from('vw_invoice as i');
    ($year) ? $CI->db->where('i.invoice_year', $year) : null;
    $prop_code = explode(" ", $property_code);
    $this->db->where_in('i.property_code',$prop_code);
