...
class Business_model extends CI_Model
{
    ...
    //	get display category and display on the add business occument form.
    public function get_display_category()
    {
        $this->db->select('name');
        $this->db->from('display_category');
        return $this->db->get()->result();
    }
    ...
    {
        $this->db->select('class_code,class');
        $this->db->from('property_class');
    ...
    public function get_zone()
    {
        ...
        $this->db->from('property_zone');
        $zone = $this->db->get()->result();
        return ($zone);
    }

    //  get class from db
    public function get_usecode()
    {
        $this->db->select('code,name');
        $this->db->from('property_type');
        $usecode = $this->db->get()->result();
        return ($usecode);
    }
    ...
    function getBusinessProperties($postData = null)
    {
        ...
        ## Fetch records
        $this->db->select(
            "buisness_property.buis_prop_code,buisness_property.property_type,buisness_property.id,".
            "po.firstname,po.lastname,po.primary_contact,po.email,".
        ...
        foreach ($records as $record) {
            ...
            #category
            $cat = ($record->property_type == 12)? "Business": "Residential";
