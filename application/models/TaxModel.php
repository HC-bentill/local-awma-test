...
class TaxModel extends CI_Model
{
    ...
    public function batch_exit($product, $category1, $year, $electoral_area, $town, $division)
    {
        ...
        ($division) ? $this->db->where('division', $division) : null;  
        $products = $this->db->get()->row_array()['count'];
    ...
    //get special batch count
    public function get_special_batch_count()
    {

        $this->db->select('count(*) as count');
        $this->db->from('special_batch_print_invoice');
        $batch_no = $this->db->get()->row_array()['count'];
        return ($batch_no);
    }
    ...
    {
        $this->db->select('b.batch_no,b.property_category,b.product as product_id,b.division,b.category1 as category1_id,b.year,b.electoral_area as area_id,b.town as town_id,t.town as town ,a.name as area,r.name, r.target,c.name as category1,b.datetime_created,b.id as bi_id');
    ...
    public function get_special_batches(){
        $this->db->select('s.*,r.name as name');
        $this->db->from('special_batch_print_invoice as s');
        $this->db->join('revenue_product as r', 'r.id = s.product', 'left');
        $special_batch = $this->db->get()->result();
        return ($special_batch);

    }

    //	get all prop codes from vw_invoice
	public function get_all_prop_codes($postdata)
	{
		$this->db->select('id,property_code');
		$this->db->from('vw_invoice');
		$this->db->where('product_id', $postdata['product']);
    ...
    public function get_batch_invoices_old($property_category, $year, $electoral_area, $town,$offset)
    {
        ...
        $this->db->select(
			...
			b.buis_occ_code, b.buis_primary_phone, b.bus_category,p.buis_prop_code,rp.res_code,
			concat(prop.firstname, " ", prop.lastname) as roName, 
    ...
    //    get all invoices to be printed
    public function get_batch_invoices($product, $category1, $year, $electoral_area, $town,$offset, $division)
    {
        $this->db->select('i.*,t.town,b.bus_category');
        $this->db->from('vw_invoice as i');
        $this->db->join('town as t', 'i.town_id = t.id', 'left');
        $this->db->join('buisness_occ as b', 'i.property_id = b.id', 'left');
        $this->db->where('i.invoice_amount <> 0');
        ($product) ? $this->db->where('i.product_id', $product) : null;
        ...
        ($division) ? $this->db->where('i.division', $division) : null;
        #$this->db->order_by("division", 'desc');
        ($product == 1) ? $this->db->order_by('property_code', 'asc') : null;
     ...
     //    get all special invoices to be printed by year and property codes stated
     public function get_special_batch_invoices($where_in,$year)
     {
         $this->db->select('i.*,t.town,b.bus_category');
         $this->db->from('vw_invoice as i');
         $this->db->join('town as t', 'i.town_id = t.id', 'left');
        ...
        //$this->db->where('i.invoice_amount <> 0');
        ($year) ? $this->db->where('i.invoice_year', $year) : null;
         $this->db->where_in('i.property_code',$where_in);
    ...
    public function get_invoice_detail_old($id)
    {
        $this->db->select(
			...
			b.buis_occ_code,b.buis_property_code, b.buis_primary_phone, b.bus_category, p.buis_prop_code, t.town,
			tt.town as occ_town,
			concat(prop.firstname, " ", prop.lastname) as roName,
            concat(po.firstname, " ", po.lastname) as boName,
    ...
    //  get divisions from db
    public function get_divisions()
    {
        $this->db->select('division');
        $this->db->from('division');
        $divisions = $this->db->get()->result();
        return ($divisions);
    }

    //get all property codes from vw_invoice
    public function get_property_codes(){
        $this->db->select('property_code');
        $this->db->from('vw_invoice');
        $property_codes = $this->db->get()->result();
    ...
    // save special batch print invoice in db
    public function save_special_batch($data)
    {
        $insert = $this->db->insert('special_batch_print_invoice', $data);
        return $this->db->insert_id();
    }
