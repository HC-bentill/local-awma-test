...
class Api extends REST_Controller {
	...
	public function codeSearch_post()
	{
		...
		if($search_category == "pn"){
			...
			if($id){
				if($category == "res"){
					$query = $this->db->query("SELECT residence.id as id,res_code as code, t.town, a.name, 'res' as flag  FROM residence left join residence_to_owner r on residence.id = r.property_id
					...
					$query1 = $this->db->query("SELECT residence.id, res_code as code, t.town, a.name, 'res' as flag FROM residence left join residence_to_owner r on residence.id = r.property_id
										left join town t on residence.town = t.id left join area_council a on residence.area_council = a.id WHERE r.owner_id = ?",[$id])->result();
					...
					$query2 = $this->db->query("SELECT b.id, buis_prop_code as code, t.town, a.name, 'busprop' as flag FROM buisness_property b left join buis_prop_to_owner r on b.id = r.property_id
										left join town t on b.town = t.id left join area_council a on b.area_council = a.id WHERE r.owner_id = ?",[$id])->result();
			...
			if($category2 == "res"){
				$query = $this->db->query("SELECT firstname,lastname,primary_contact FROM residence left join residence_to_owner r on residence.id = r.property_id
									left join property_owner p on r.owner_id = p.id WHERE residence.res_code = ?",[$property_code]);
				...
				$query = $this->db->query("SELECT firstname,lastname,primary_contact FROM buisness_property b left join buis_prop_to_owner r on b.id = r.property_id
									left join property_owner p on r.owner_id = p.id WHERE b.buis_prop_code = ?",[$property_code]);
				if($query->num_rows() > 0){
					$this->set_response($query->result(), REST_Controller::HTTP_OK);
				}else{
					$this->set_response("Empty", REST_Controller::HTTP_OK);
	...
	public function incomplete_post()
	{
		...
		if($category === "res"){
			$query = $this->db->query("SELECT res_code as code, t.town, a.name, residence.gps_lat, residence.gps_long FROM residence left join town t on residence.town = t.id
							left join area_council a on residence.area_council = a.id WHERE residence.agent_id = ? AND residence.status= ?",[$agentId,0])->result();
		}elseif($category === "busprop"){
			$query = $this->db->query("SELECT buis_prop_code as code, t.town, a.name, b.gps_lat, b.gps_long FROM buisness_property b left join town t on b.town = t.id
							left join area_council a on b.area_council = a.id WHERE b.agent_id = ? AND b.status= ?",[$agentId,0])->result();
		}else{
			$query1 = $this->db->query("SELECT res_code as code, t.town, a.name, residence.gps_lat, residence.gps_long  FROM residence left join town t on residence.town = t.id
							left join area_council a on residence.area_council = a.id WHERE residence.agent_id = ? AND residence.status= ?",[$agentId,0])->result();
...
// save business occupant
	...
	//	add new residence
	public function saveResidence_post(){
	...
	public function saveBusiness_post(){
		...
		if(get_channel_status(5) == 0){
			...
			$code = $this->bus->resnumber_new($this->post('areaCouncil'),$this->post('town'));
			$primary_contact = trim($this->post('ownerPrimaryContact'));
