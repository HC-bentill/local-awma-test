...
class Business extends CI_Controller {
	...
	public function edit_business_property_form($id,$propcode){
		...
		if(has_permission($this->session->userdata('user_info')['id'],'manage buis prop')){
			$data = array(
				...
				'construction' => $this->res->get_cons(),
				'roof' => $this->res->get_roof(),
				'invoices' => $this->res->get_property_invoice($id,2),
				'invoices_sum' => $this->res->get_property_invoice_sum($id,2),
				'bread_crumbs' => $breadCrumbs
			);
		...
		if($update == "update"){
			...
			if($primary_contact == $original_primary_contact){
				...
				if(owner_exit(trim($this->input->post('primary_contact')))){
					...
					$owner_id = $this->res->update_busocc_to_owner($id,$id);
				}else{
	...
	public function import_arrears_records()
	{
		set_time_limit(0);
				...
				if (is_uploaded_file($_FILES['userfile']['tmp_name'])) {
					...
					while (($line = fgetcsv($csvFile)) !== FALSE) {
						...
						$data = array(
							...
							'property_id' => $id,
							'product_id' => 1,
							'category1_id' => trim($line[4]),
							'category2_id' => trim($line[5]),
							'category3_id' => trim($line[6]),
							'category4_id' => trim($line[7]),
							'category5_id' => trim($line[8]),
							'category6_id' => trim($line[9]),
							'invoice_amount' => trim($line[13]),
							'invoice_year' => $year,
							'amount_paid' => trim($line[16])
						);
	...
	public function import_property_records()
	{
		set_time_limit(0);
		$count = 0;
		$successful = 0;
		...
		//open uploaded csv file with read only mode
		$csvFile = fopen($_FILES['userfile']['tmp_name'], 'r') or die("can't open file");
		...
		while ($line = fgetcsv($csvFile)) {
			...
			$primary_contact = trim($line[2]);

			$owner_exit = owner_exit(trim($line[2]));

			$owner['firstname'] = ucfirst(trim($line[3]));
			$owner['person_category'] = "Owner";
			$owner['primary_contact'] = trim($line[2]);

			$data1['buis_prop_code'] = trim($line[14]); // replace with old_property_code

			$data1['town'] = trim($line[1]);
			$data1['area_council'] = trim($line[0]);
			$data1['division'] = trim($line[4]);
			$data1['block_no'] = trim($line[22]);
			$data1['plot_no'] = trim($line[23]);
			$data1['category'] = 12;
			$data1['property_typee'] = trim($line[24]);
			$data1['property_category'] = trim($line[25]);
			$data1['streetname'] = trim($line[13]);

			$data1['new_property_no'] = $gen_rescode;
			$data1['old_property_no'] = trim($line[14]);

			$data1['accessed'] = trim($line[15]);
			$rateable_amount = trim($line[16]);
			$rate = trim($line[17]);

			$data1['code'] = $code;
			$data1["agent_id"] = $this->session->userdata('user_info')['id'];
			$data1["agent_category"] = "admin";
			...
			if ($owner_id) {
				$bus_id = $this->res->add_business($data1);
				...
				$category['category1'] = trim($line[5]);
				$category['category2'] = trim($line[6]);
				$category['category3'] = trim($line[7]);
				$category['category4'] = trim($line[8]);
				$category['category5'] = trim($line[9]);
				$category['category6'] = trim($line[10]);
				$category['property_id'] = $bus_id;
				$category_insert = $this->db->insert('busprop_to_category', $category);
	...
	public function import_property_arrears()
	{
		set_time_limit(0);
		...
		if (!$this->upload->do_upload()) {
			...
			if (!empty($_FILES['userfile']['tmp_name']) && in_array($_FILES['userfile']['type'], $csvMimes)) {
				$file_data = $this->upload->data();
				if (is_uploaded_file($_FILES['userfile']['tmp_name'])) {
					...
					while (($line = fgetcsv($csvFile)) !== FALSE) {
						...
						$year = 2020;
						$code = $this->TaxModel->get_code(12,$year);
						$final_code = $code + 1;
						$number = str_pad($final_code, 5, '0', STR_PAD_LEFT);
						$invoice_no = "INVNBPR".$year."-".$number;
						$today =  date('Y-m-d');
						...
						$data = array(
							...
							'invoice_amount' => trim($line[18]),
							'invoice_year' => $year,
							'amount_paid' => trim($line[21])
						);
						$insert = $this->TaxModel->insert_invoice_record($data);
						$id++;
					}
